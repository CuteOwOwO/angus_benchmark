...
type FinalHandoffVariant =
  ...
  | "final_function_response_plus_ready_signal"
...
async function runOne(ai: GoogleGenAI, model: string, plan: AttemptPlan): Promise<AttemptSummary> {
  ...
  const sendFinalResponse = (call: FunctionCall) => {
    ...
    if (
      plan.variant === "final_function_response_plus_ready_signal" ||
      plan.variant === "final_function_response_plus_full_result_client_message"
    ) {
      timers.push(
        setTimeout(() => {
          ...
          const message =
            plan.variant === "final_function_response_plus_full_result_client_message"
              ? fullResultClientMessage()
              : finalReadyClientMessage();
          const eventType =
            plan.variant === "final_function_response_plus_full_result_client_message"
              ? "final_full_result_client_message_sent"
              : "final_ready_client_message_sent";
          try {
            session?.sendClientContent({ turns: message, turnComplete: true });
            state.finalClientMessageSentAt = Date.now();
            appendTimeline(eventType, { message });
            appendJsonl(rawLogPath, {
              type: eventType,
              event_ms: state.promptSentAt ? Date.now() - state.promptSentAt : null,
              message,
  ...
  const scheduleFinalResponse = (call: FunctionCall) => {
    timers.push(
      setTimeout(() => {
        if (plan.variant === "full_result_client_message_only") {
          if (done || state.sessionClosed || state.finalHandoffAt) return;
          try {
            const message = fullResultClientMessage();
            session?.sendClientContent({ turns: message, turnComplete: true });
            state.finalClientMessageSentAt = Date.now();
            state.finalHandoffAt = state.finalClientMessageSentAt;
            appendTimeline("final_full_result_client_message_sent", { message });
            appendJsonl(rawLogPath, {
              type: "final_full_result_client_message_sent",
              event_ms: state.promptSentAt ? Date.now() - state.promptSentAt : null,
              message,
            });
          } catch (error) {
            noteSendError("final_full_result_client_message", error);
          }
          return;
        }
        {
...
function writeFinalMarkdown(path: string, rows: VariantSummary[], resultDir: string): void {
  const lines = [
    "# Gemini Live final result channel probe",
    "",
    `Result folder: ${resultDir}`,
    "",
    "| variant | attempts | 1008 | 1011 | final_function_response | final_client_message | post_final_model_event | post_final_audio | post_final_text | final_answer | turn_before_final | turn_after_final |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ...rows.map(
      (row) =>
        `| ${row.condition} | ${row.attempts} | ${row.close_1008_count} | ${row.close_1011_count} | ${row.final_function_response_sent_count} | ${row.final_client_message_sent_count} | ${row.any_model_event_after_final_count} | ${row.first_audio_after_final_count} | ${row.first_text_or_transcription_after_final_count} | ${row.post_tool_final_answer_count} | ${row.turnComplete_before_final_count} | ${row.turnComplete_after_final_count} |`,
    ),
    "",
    "Interpretation:",
    "",
    "This small probe keeps the with_tick_external path fixed and compares whether the final result is better handed off as a native function response, a client ready signal, a full client result message, or a full client result message without a final function response. Do not treat these 5-attempt cells as final stability estimates.",
    "",
  ];
  writeFileSync(path, `${lines.join("\n")}\n`, "utf8");
}
...
async function main(): Promise<void> {
  ...
  const resultId = `${timestampForPath()}_tau_live_tool_final_result_channel_probe`;
