...
type FinalHandoffVariant =
  | "natural_waiting_external_tick"
  | "quiet_ack_only_external_tick";
...
const QUIET_PROMPT_NAME = "taw-with-tick-quiet-ack-only";
const QUIET_SYSTEM_INSTRUCTION = `You are a helpful voice assistant.

The user asks a task that requires using the provided tool.
You must call the tool to get the answer.

After calling the tool, you may say at most one brief acknowledgement such as "Sure, I am checking that now."

While the tool is pending:
- Do not provide repeated waiting updates.
- Do not respond to pending status ticks.
- Do not say that the lookup is still in progress more than once.
- Do not claim any order status, delivery state, delay reason, or final result.

Pending status may appear as [TOOL_PENDING_STATUS].
Pending status only means the tool is still running and no final result is available yet.
...
Important:
Do not mechanically respond to pending signals.
Do not continue speaking just to fill the wait.
Once the final result arrives, answer immediately.`;
...
type AttemptState = {
  ...
  finalHandoffAt?: number;
  finalFunctionResponseSentAt?: number;
  finalClientMessageSentAt?: number;
...
type AttemptSummary = {
  ...
  final_client_message_sent: boolean;
  final_client_message_sent_time_ms: number | null;
...
type VariantSummary = {
  ...
  final_client_message_sent_count: number;
...
function parseCsvVariants(value: string | undefined): FinalHandoffVariant[] {
  ...
  for (const variant of variants) {
    if (!DEFAULT_VARIANTS.includes(variant as FinalHandoffVariant)) throw new Error(`Unknown waiting speech variant: ${variant}`);
  }
...
function promptForVariant(variant: FinalHandoffVariant): { promptName: string; systemInstruction: string; userPrompt: string } {
  if (variant === "quiet_ack_only_external_tick") {
    return {
      promptName: QUIET_PROMPT_NAME,
      systemInstruction: QUIET_SYSTEM_INSTRUCTION,
...
function pendingPayload(): Record<string, unknown> {
  return {
    phase: "pending",
    message: "The order lookup is still running.",
    final_result_available: false,
  };
}

function clientPendingMessage(): string {
  return ["[TOOL_PENDING_STATUS]", "phase: pending", "The order lookup is still running.", "No final result is available yet."].join("\n");
}
...
function finalReadyClientMessage(): string {
  return ["[TOOL_FINAL_RESULT_READY]", "The final tool result is now available. Answer the user now."].join("\n");
}

function fullResultClientMessage(): string {
  return [
    "[TOOL_FINAL_RESULT]",
    "phase: final",
    "order_id: A123",
...
function computeSummary(plan: AttemptPlan, state: AttemptState): AttemptSummary {
  ...
  return {
    ...
    session_valid: !close1008 && !close1011 && Boolean(state.finalHandoffAt) && postToolFinalAnswer,
    ...
    final_client_message_sent: Boolean(state.finalClientMessageSentAt),
    final_client_message_sent_time_ms: msDelta(state.promptSentAt, state.finalClientMessageSentAt),
...
async function runOne(ai: GoogleGenAI, model: string, plan: AttemptPlan): Promise<AttemptSummary> {
  ...
  const prompt = promptForVariant(plan.variant);
  ...
  const sendFinalResponse = (call: FunctionCall) => {
    if (done || state.sessionClosed || state.finalHandoffAt) return;
    if (sendToolResponse(call, finalPayload(), "final_tool_response_sent")) {
      state.finalFunctionResponseSentAt = Date.now();
      state.finalHandoffAt = state.finalFunctionResponseSentAt;
    }
  };
        ...
        callbacks: {
          ...
          onmessage: (message: LiveMessage) => {
            ...
            if (message.serverContent?.turnComplete) {
              ...
              if (state.finalHandoffAt) state.turnCompleteAfterFinal = true;
              else state.turnCompleteBeforeFinal = true;
              appendTimeline("turn_complete", { phase: state.finalHandoffAt ? "after_final" : "before_final" });
              if (state.finalHandoffAt) {
          ...
          onclose: (event: { code?: number; reason?: string }) => {
            ...
            state.sessionClosesBeforeAnyPostFinalModelEvent = Boolean(state.finalHandoffAt && !state.anyModelEventAfterFinal);
...
function summarizeVariant(condition: FinalHandoffVariant, attempts: AttemptSummary[]): VariantSummary {
  return {
    ...
    final_client_message_sent_count: attempts.filter((attempt) => attempt.final_client_message_sent).length,
...
function writeFinalMarkdown(path: string, rows: VariantSummary[], resultDir: string): void {
  const lines = [
    "# Gemini Live waiting speech ablation probe",
    "",
    `Result folder: ${resultDir}`,
    "",
    "| variant | attempts | 1008 | 1011 | final_function_response | post_final_model_event | post_final_audio | post_final_text | final_answer | turn_before_final | turn_after_final |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ...rows.map(
      (row) =>
        `| ${row.condition} | ${row.attempts} | ${row.close_1008_count} | ${row.close_1011_count} | ${row.final_function_response_sent_count} | ${row.any_model_event_after_final_count} | ${row.first_audio_after_final_count} | ${row.first_text_or_transcription_after_final_count} | ${row.post_tool_final_answer_count} | ${row.turnComplete_before_final_count} | ${row.turnComplete_after_final_count} |`,
    ),
    "",
    "Interpretation:",
    "",
    "This small probe keeps native tool call, external pending ticks, and final functionResponse fixed. It compares whether natural pending speech versus a quiet ack-only pending policy changes post-final answer behavior. Do not treat these 5-attempt cells as final stability estimates.",
    "",
  ];
  writeFileSync(path, `${lines.join("\n")}\n`, "utf8");
}
...
async function main(): Promise<void> {
  ...
  const resultId = `${timestampForPath()}_tau_live_tool_waiting_speech_ablation_probe`;
  ...
  writeJson(resolve(resultDir, "config.json"), {
    ...
    prompt_variants: {
      ...
      quiet_ack_only_external_tick: {
        prompt_name: QUIET_PROMPT_NAME,
        system_instruction: QUIET_SYSTEM_INSTRUCTION,
        user_prompt: WITH_TICK_USER_PROMPT,
      },
    },
    pending_payload: pendingPayload(),
